#!/usr/bin/env python3
"""
DiskANN In-Memory Benchmark: Compare Caliby DiskANN vs Microsoft DiskANN (diskannpy)

This benchmark compares two DiskANN implementations using IN-MEMORY indices:
- Caliby: Buffer-managed DiskANN with disk persistence
- Microsoft DiskANN (diskannpy): Official Microsoft implementation with StaticMemoryIndex
...
try:
    import caliby
    caliby.set_buffer_config(size_gb=8.0)  # Larger buffer for in-memory workloads
...
def load_dataset(data_dir='./deep10M', max_vectors=None, use_sift=False):
    """Load dataset (DEEP10M or SIFT1M)."""
    # Auto-detect dataset type if not specified
    if not use_sift:
        # Check if SIFT files exist
        sift_base = os.path.join(data_dir, 'sift_base.fvecs')
        if os.path.exists(sift_base):
            print(f"\nAuto-detected SIFT1M dataset in {data_dir}")
            use_sift = True
    ...
    if use_sift:
        ...
        if not base_file or not query_file or not groundtruth_file:
            ...
            print("  - deep10M_base.fbin or base.10M.fbin")
            print("  - deep10M_query.fbin or query.public.10K.fbin")
            print("  - deep10M_groundtruth.ibin or groundtruth.ibin")
            sys.exit(1)
...
def benchmark_caliby_diskann(base_vectors, query_vectors, groundtruth, params, data_dir):
    ...
    print("\n" + "="*70)
    print("Benchmarking Caliby DiskANN (In-Memory Mode)")
    print("="*70)
    ...
    index_dir = os.path.join(data_dir, 'caliby_diskann_mem_index')
    ...
    # Benchmark single query search
    print(f"Running single-query search benchmark (L_search={params['L_search']})...")
    latencies = []
    all_results = []
...
def benchmark_caliby_batch(base_vectors, query_vectors, groundtruth, params, data_dir):
    ...
    index_dir = os.path.join(data_dir, 'caliby_diskann_batch_index')
    
    # Note: Caliby DiskANN doesn't support loading existing index from disk
    # We must rebuild for batch benchmark
    print("Building index for batch search...")
    if os.path.exists(index_dir):
        shutil.rmtree(index_dir)
    os.makedirs(index_dir)
...
def benchmark_diskannpy_memory(base_vectors, query_vectors, groundtruth, params, data_dir):
    """Benchmark Microsoft DiskANN (diskannpy) using StaticMemoryIndex."""
    print("\n" + "="*70)
    print("Benchmarking Microsoft DiskANN (diskannpy) - StaticMemoryIndex")
    print("="*70)
    ...
    # Create directory for index
    index_dir = os.path.join(data_dir, 'diskannpy_mem_index')
    if os.path.exists(index_dir):
        shutil.rmtree(index_dir)
    os.makedirs(index_dir)
    
    print(f"\nBuilding index with {num_vectors} vectors (dim={dim})...")
    print(f"  R={params['R']}, complexity={params['L_build']}, alpha={params['alpha']}")
    
    start_time = time.time()
    
    # Build memory index using diskannpy
    diskannpy.build_memory_index(
        ...
        num_threads=params.get('num_threads', 0),
        alpha=params['alpha'],
        use_pq_build=False,  # No PQ compression for in-memory
        index_prefix='ann'
    )
    ...
    print("\nLoading StaticMemoryIndex...")
    index = diskannpy.StaticMemoryIndex(
        index_directory=index_dir,
        num_threads=params.get('num_threads', 0),
        initial_search_complexity=params['L_search'],
        index_prefix='ann'
    )
    ...
    # Benchmark single query search
    print(f"Running single-query search benchmark (L_search={params['L_search']})...")
    print("Note: Using batch_search with single queries due to diskannpy.search() returning duplicates")
    latencies = []
    all_results = []
    
    for i in range(num_queries):
        start = time.perf_counter()
        # Use batch_search with single query instead of search() due to duplicate bug
        result = index.batch_search(
            queries=query_vectors[i:i+1],
            k_neighbors=10,
            complexity=params['L_search'],
            num_threads=1  # Single query, single thread
        )
        latency = (time.perf_counter() - start) * 1000  # ms
        latencies.append(latency)
        all_results.append(result.identifiers[0].astype(np.int32))
    ...
    print(f"\nDebug info (diskannpy Memory):")
    print(f"  Results shape: {results_array.shape}")
    print(f"  Sample result[0]: {results_array[0, :10]}")
    print(f"  Groundtruth shape: {groundtruth.shape}")
    print(f"  Sample groundtruth[0]: {groundtruth[0, :10]}")
    
    recall = compute_recall_at_k(results_array, groundtruth, k=10)
    
    return BenchmarkResult(
        library='diskannpy (Memory)',
        build_time=build_time,
        index_size_mb=index_size_mb,
        qps=qps,
        latency_p50=np.percentile(latencies, 50),
...
def benchmark_diskannpy_memory_batch(base_vectors, query_vectors, groundtruth, params, data_dir):
    """Benchmark Microsoft DiskANN (diskannpy) using StaticMemoryIndex batch search."""
    print("\n" + "="*70)
    print("Benchmarking Microsoft DiskANN (diskannpy) - Memory Batch Search")
    print("="*70)
    ...
    # Load the index
    print("Loading StaticMemoryIndex for batch search...")
    index = diskannpy.StaticMemoryIndex(
        index_directory=index_dir,
        num_threads=params.get('num_threads', 0),
        initial_search_complexity=params['L_search'],
...
def print_results_table(results: List[BenchmarkResult]):
    ...
    print("\n" + "="*110)
    print("BENCHMARK RESULTS SUMMARY (In-Memory)")
    print("="*110)
...
def main():
    parser = argparse.ArgumentParser(description='DiskANN In-Memory Benchmark: Caliby vs Microsoft DiskANN')
    parser.add_argument('--data-dir', default='./benchmark/deep10M', help='Directory for dataset')
    parser.add_argument('--use-sift', action='store_true', help='Use SIFT1M instead of DEEP10M')
    parser.add_argument('--max-vectors', type=int, default=None, help='Limit number of base vectors')
    ...
    print("="*110)
    print("DISKANN IN-MEMORY BENCHMARK: Caliby vs Microsoft DiskANN (diskannpy) StaticMemoryIndex")
    print("="*110)
    ...
    if args.use_sift:
        data_dir = args.data_dir if args.data_dir != './benchmark/deep10M' else './benchmark/sift1m'
    else:
        data_dir = args.data_dir
    
    # Load data
    base_vectors, query_vectors, groundtruth = load_dataset(
        data_dir, 
        max_vectors=args.max_vectors,
        use_sift=args.use_sift
    )
    
    # Update dataset name based on actual data loaded
    if base_vectors.shape[1] == 128:
        dataset_name = "SIFT1M"
    elif base_vectors.shape[1] == 96:
        dataset_name = "DEEP10M"
    print(f"Loaded: {base_vectors.shape[0]} vectors, dimension={base_vectors.shape[1]}")
    ...
    if not args.batch_only:
        ...
        if enabled_libs['diskannpy']:
            try:
                result = benchmark_diskannpy_memory(base_vectors, query_vectors, groundtruth, params, data_dir)
                results.append(result)
            except Exception as e:
                print(f"\n✗ diskannpy memory benchmark failed: {e}")
                import traceback
                traceback.print_exc()
    ...
    if enabled_libs['diskannpy']:
        try:
            result = benchmark_diskannpy_memory_batch(base_vectors, query_vectors, groundtruth, params, data_dir)
            results.append(result)
        except Exception as e:
